...
@export var overlay_open_sfx: AudioStream
@export var overlay_close_sfx: AudioStream
...
const card_scene: PackedScene = preload(
	"res://src/card.tscn"
)
...
func _ready() -> void:
	%SkipArtifactsButton.pressed.connect(
		close
	)
...
func open() -> void:
	...
	visible = true
	play_sfx(overlay_open_sfx, true)
...
func close() -> void:
	...
	play_sfx(overlay_close_sfx, true)
	...
	for card in GlobalManager.deck:
		...
		if use_duplicate_cards:
			...
			if card.get_parent() != null:
				card.get_parent().remove_child(
					card
				)

		display_card.apply_scene_size()
		display_card.rotation = 0.0
		...
		if not display_card.pressed.is_connected(
			on_card_pressed
		):
			display_card.pressed.connect(
				on_card_pressed
			)


func make_duplicate_card(
	card: Card
) -> Card:
	var display_card := (
	...
	display_card.show_zero_on_damage = (
		card.show_zero_on_damage
	)

	if card.has_stamp and card.stamp != null:
		var stamp_copy := (
			card.stamp.duplicate()
			as Stamp
		)

		if stamp_copy != null:
			display_card.set_stamp(
				stamp_copy
			)
...
func destroy_card(
	...
	var original_card: Card = (
		_duplicate_originals.get(
			display_card,
			display_card
		)
	)

	GlobalManager.deck.erase(
		original_card
	)

	_original_parents.erase(
		original_card
	)

	_duplicate_originals.erase(
		display_card
	)
	...
	if (
		original_card != display_card
		and is_instance_valid(original_card)
	):
...
func stamp_card(
	...
	var real_stamp := (
		_stamp_scene.instantiate()
		as Stamp
	)

	if real_stamp != null:
		original_card.set_stamp(
			real_stamp
		)
